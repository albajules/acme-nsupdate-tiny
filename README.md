...
your private Let's Encrypt account key and Cloudflare API Token, I tried to make it
as tiny as possible (currently less than 100 lines).
The only prerequisites are python, openssl.
...
acme-cloudflare-tiny, to register an account for you and sign all following requests.
...
[JWK](https://tools.ietf.org/html/rfc7517) format. `acme-cloudflare-tiny` is using the PEM
...
requires you add a TXT record to your domain. This script will issue Cloudflare API
requests to your domain. You need to create an API Token with "Zone:DNS:Edit" permission.
...
```
# Run the script
python acme_cloudflare_tiny.py --account-key ./account.key --csr ./domain.csr --cf-token "YOUR_CLOUDFLARE_API_TOKEN"
```
...
```sh
#!/usr/bin/sh
CRT=$(python /path/to/acme_cloudflare_tiny.py --account-key /path/to/account.key --csr /path/to/domain.csr --production --cf-token "YOUR_TOKEN" || exit)
